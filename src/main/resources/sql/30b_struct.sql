    ...
    WHERE sbs.deleted ISNULL
     ...
     bfs_pre AS (
         SELECT
             ...
             (CASE WHEN rep_category_type = 'NC' THEN fill_seed_fund ELSE 0 END)              AS rp_NC,
             (CASE WHEN rep_category_type = 'OS' THEN fill_seed_fund ELSE 0 END)              AS rp_OS,
             (CASE WHEN rep_category_type = 'ES' THEN fill_seed_fund ELSE 0 END)              AS rp_ES,
             (CASE WHEN rep_type IN ('RS', 'RS1') AND rep_trade != TRUE THEN fill_seed_fund ELSE 0 END)        AS rp_RS_1,
             (CASE WHEN rep_type IN ('RS', 'RS1') AND rep_trade = TRUE THEN fill_seed_fund ELSE 0 END)         AS rp_RS_1_RST,
             (CASE WHEN rep_type IN ('RS2') AND rep_trade != TRUE THEN fill_seed_fund ELSE 0 END)          AS rp_RS_2,
             (CASE WHEN rep_type IN ('RS2') AND rep_trade = TRUE THEN fill_seed_fund ELSE 0 END)          AS rp_RS_2_RST,
             (CASE WHEN rep_type IN ('RS3') AND rep_trade != TRUE THEN fill_seed_fund ELSE 0 END)          AS rp_RS_3,
             (CASE WHEN rep_type IN ('RS3') AND rep_trade = TRUE THEN fill_seed_fund ELSE 0 END)          AS rp_RS_3_RST,
             (CASE WHEN rep_type IN ('RS4') AND rep_trade != TRUE THEN fill_seed_fund ELSE 0 END)          AS rp_RS_4,
             (CASE WHEN rep_type IN ('RS4') AND rep_trade = TRUE THEN fill_seed_fund ELSE 0 END)          AS rp_RS_4_RST,
             (CASE WHEN rep_type = 'F1' THEN fill_seed_fund ELSE 0 END)                AS rp_F1,
             (CASE WHEN rep_type IN ('RS5', 'RS6', 'RS7', 'RS8', 'RS9', 'RS10') AND rep_trade != TRUE THEN fill_seed_fund ELSE 0 END) AS rp_RS_5B,
             (CASE WHEN rep_type IN ('RS5', 'RS6', 'RS7', 'RS8', 'RS9', 'RS10') AND rep_trade = TRUE THEN fill_seed_fund ELSE 0 END) AS rp_RS_5B_RST,

             (CASE WHEN rep_id ISNULL OR (rep_category_type NOT IN ('NC', 'OS', 'ES') AND rep_type NOT IN ('RS', 'RS1', 'RS2', 'RS3', 'RS4', 'F1' , 'RS5', 'RS6', 'RS7', 'RS8', 'RS9', 'RS10')) THEN fill_seed_fund ELSE 0 END) AS rp_NOT,
         ...
         FROM (
                  SELECT
                      ...
                      coalesce(fill_all, 0.0) - coalesce(fill_SOWN, 0.0) - coalesce(fill_SOWN_old, 0.0) - coalesce(fill_DELETED, 0.0) - coalesce(fill_SOLD, 0.0) - coalesce(fill_DEFECTED, 0.0) - coalesce(fill_FED, 0.0) - coalesce(fill_REFORMED, 0.0) AS fill_AVALIABLE,
                  ...
                  FROM
                      (SELECT
                       ...
                       WHERE true
                         AND bfs.seedprod_for_harvest_year = :harvest_year
     ...
     sorts_w_culcats_pre_infos AS (
         SELECT
             ...
             CASE WHEN :details AND spr.count_backfills > 0 THEN jsonb_agg(cast(ROW (
                 ...
                 bfs_pre.fill_all,
                 bfs_pre.fill_seed_fund,
                 bfs_pre.fill_seed_for_sale,
                 bfs_pre.fill_removed_seed_fund,
                 bfs_pre.fill_avaliable,
                 bfs_pre.fill_sown,
                 bfs_pre.fill_deleted,
                 bfs_pre.fill_sold,
                 bfs_pre.fill_defected,
                 bfs_pre.fill_fed,
                 bfs_pre.fill_reformed,
                 bfs_pre.ru_fill_all,
